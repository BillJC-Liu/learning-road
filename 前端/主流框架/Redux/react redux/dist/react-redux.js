	...
	*/
	...
	function createListenerCollection() {
	  ...
	  return {
	    ...
	    notify: function notify() {
	      // []
	      var listeners = current = next; // batch =  defaultNoopBatch(callback){ callback() }
	      // 就是将注册的reducer进行更新
	...
	function () {
	  ...
	  _proto.trySubscribe = function trySubscribe() {
	    if (!this.unsubscribe) {
	      this.unsubscribe = this.parentSub ? this.parentSub.addNestedSub(this.handleChangeWrapper) : this.store.subscribe(this.handleChangeWrapper); // createListenerCollection 返回一个对象 
	...
	function connectAdvanced(selectorFactory, _ref) {
	  ...
	  return function wrapWithConnect(WrappedComponent) {
	    ...
	    var wrappedComponentName = WrappedComponent.displayName || WrappedComponent.name || 'Component'; // `ConnectAdvanced(Component)`
