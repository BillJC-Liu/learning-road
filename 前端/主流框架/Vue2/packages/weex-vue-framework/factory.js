...
function defineReactive$$1 (
  ...
  // 指定对象上一个自有属性对应的属性描述符。 
  // const a =  {
  //   configurable: true,     // false时，Object.defineProperty 无法修改 writable 以外的值，writable 若初始值为false，则该对象只读
  //   enumerable: true,      // 是否可枚举
  //   value: 1,              // 值
  //   writable: true         // 是否可写
  // }
  var property = Object.getOwnPropertyDescriptor(obj, key);
  // configurable 表示无法修改属性
...
function set (target, key, val) {
  ...
  // 如果该对象目标没有 __ob__ 属性，说明没有还没有被注入观察者
  // 则修改该对象对应的key  value 
  if (!ob) {
    target[key] = val;
    return val
 ...
 * @param { this } ctx  上下文 this
 */
function nextTick (cb, ctx) {
  var _resolve;
  // callbacks 数组 初始值为空数组
...
function stateMixin (Vue) {
  ...
  // 在vue实例上绑定 $data $props 可以通过 this.$data this.$props 进行访问
...
initMixin(Vue);  // 在 vue 实例上注册一个 _init方法
